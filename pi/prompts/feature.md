...
argument-hint: "<pedido del feature>"
---
...
- **Subagentes (trabajo pesado, vía `pi-subagents`)**: delega por fases — exploración profunda, especificación, plan de tareas, implementación, verificación. OJO: los roles con nombre del entorno OpenCode (`sdd-explore`, `sdd-spec`, `sdd-apply`, …) NO existen como definiciones instaladas en Pi; con `pi-subagents` cada subagente es genérico, así que el ROL va en el propio prompt de delegación (instrucciones de la fase + alcance + qué devolver). Tú orquestas y aplicas los gates; los subagentes hacen el trabajo. Cada prompt de subagente es autocontenido e incluye las convenciones del AGENTS.md y los constraints VERBATIM del usuario (los subagentes no tienen memoria de esta conversación).
...
- **Review adversarial (cambios grandes)**: si el diff estimado supera ~400 líneas o el cambio es de alto riesgo, lanza DOS subagentes genéricos (vía `pi-subagents`) como jueces ciegos: cada uno recibe el diff y el mismo prompt de revisión adversarial (busca bugs, regresiones y drift de alcance; reporta hallazgos con severidad), SIN ver el trabajo del otro, y con la evidencia de tests/YATT/build adjunta. Si la extensión de subagentes no está disponible, hacé la revisión dual tú mismo en dos pasadas separadas y declaralo así en el resultado. Los hallazgos llevan máximo una ronda de fix acotada y re-juzgamiento, dentro del GATE 2.
- **Git es del usuario**: nunca `git commit`, `git push` ni PRs. Dejá el working tree verificado y reportá qué cambió.

## Setup

- Working directory: ejecuta `git rev-parse --show-toplevel 2>/dev/null || pwd` antes que nada y usa esa ruta como workspace (en algunos launchers la interpolación resuelve al directorio de datos de la app, no al proyecto).
...
> **Pi**: las herramientas `yatt_*` existen solo si el server MCP de YATT está registrado (pi-mcp-adapter + `bun run ~/.local/share/yatt/mcp/src/server.ts --root ~/.local/share/yatt`). Si no están disponibles, salta las secciones YATT y marca toda la verificación E2E como ⚠️ diferida en `verification.md` — nunca la simules.
...
- Corre SOLO los tests afectados; la suite completa solo en el cierre final o si el cambio cruza módulos. Antes de declarar una tarea cerrada: build/typecheck REAL del repo tocado en verde (`/droxon-verify` o el comando equivalente vía bash; un `tsc --noEmit` pelado no cuenta para Angular ni meta-frameworks).
...
3. Verificación final por capa:
   - Build del proyecto + tests afectados en verde (suite completa si cruza módulos) + gate de build REAL (`/droxon-verify`) de TODOS los repos tocados en verde. Nada queda rojo.
   - **Review adversarial si aplica**: cambio grande (>~400 líneas) o de alto riesgo → pasada dual ciega con DOS subagentes genéricos como jueces (mismo mecanismo del punto de delegación: los roles jd-judge NO están instalados en Pi, el prompt de juez viaja en la delegación), con la evidencia adjunta; máximo una ronda de fix acotada sobre hallazgos confirmados.
